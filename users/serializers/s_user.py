...
class UserSerializer(HyperlinkedModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if settings.ENVIRONMENT != "localhost":
            subject = "Activa tu cuenta de Flowers Store"
            from_email = settings.EMAIL_HOST_USER
            to = user.email
            text_content = "Para continuar, por favor activa tu cuenta de Flowers Store en el siguiente <a href=activate/>link.</a>"
            html_content = """
                <h2>Bienvenido a Flowers Store {0}!</h2>
                <p>
                    Para continuar, por favor activa tu cuenta de Flowers Store con el siguiente
                    <a href="{1}activate/{2}">link.</a>
                </p>
                <span>El equipo de Flowers Store.</span>
                <br/>
            """.format(
                user.first_name,
